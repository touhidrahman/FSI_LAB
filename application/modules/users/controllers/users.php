...
class Users extends MX_Controller
{
    ...
    function password_ck($password)
    {
        ...
        $this->load->model('mdl_user');
        $result = $this->mdl_user->password_ck($username, $password);
        if ($result == FALSE)
        {
    ...
    function registration()
    {
        ...
        $this->form_validation->set_rules('username', 'Username', 'trim|required|xss_clean|valid_email|callback_is_unique');
        ...
        if ($this->form_validation->run($this) == FALSE) {
            ...
            $data['password'] = $this->site_security->make_hash($pass);
            $data['username'] = $this->input->post('username');
            
            $data['user_id'] = $this->_insert($data);   //returns insert id
            $this->session->set_userdata('user_id', $data['user_id']);
            $this->session->set_flashdata('notify', 'New User Successfully Created!');
            
            $data['module'] = "profiles"; 
            $data['view_file'] = "new_profile";
            $this->load->module('template');
            $this->template->public_one_col($data);
        }
    }
    ...
    {
        $q = $this->get_where_custom('username', $email);
        if ($q->num_rows() > 0)
        {
            $this->form_validation->set_message('is_unique', "An account with $email already exists.");
            return FALSE;
        }
        return TRUE;
    }
